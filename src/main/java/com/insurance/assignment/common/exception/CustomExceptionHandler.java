...
public class CustomExceptionHandler {
    ...
    public ResponseEntity<MessageResponse> handleValidationException(
        ...
        MessageResponse response = new MessageResponse(CONSTANTS.HTTP_RESPONSE.STATUS_FAIL, String.format(errorMessages));
        return new ResponseEntity<>(response, HttpStatus.BAD_REQUEST);
    }

    public ResponseEntity<MessageResponse> handleHttpMessageNotReadable(HttpMessageNotReadableException ex, HttpHeaders headers, HttpStatus status, WebRequest webRequest) throws BusinessException {
        log.error(Throwables.getStackTraceAsString(ex));
        String message = ex.getMessage();
        MessageResponse response = new MessageResponse(CONSTANTS.HTTP_RESPONSE.STATUS_FAIL, message);
